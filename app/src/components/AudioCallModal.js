...
const AudioCallModal = ({
  ...
  const createRoom = async () => {
    try {
      ...
      remoteStreamRef.current = { tracks: [] };
      pc.ontrack = (event) => {
        remoteStreamRef.current.tracks.push(...event.streams[0].getTracks());
      };
        ...
        <Modal.Body flex={1} justifyContent="center" alignItems="center">
          {recipientDetails && (
            <VStack space={6} alignItems="center">
              ...
              <HStack space={8} mt={10}>
                {/* Hang Up Button */}
                <Pressable onPress={hangUp}>
                  <Box bg="red.500" borderRadius="full" p={4} shadow={3}>
                    <Icon
                      as={MaterialCommunityIcons}
                      name="phone-hangup"
                      size="xl"
                ...
                {callState === "calling" && caller === "recipient" ? (
                  <Pressable onPress={() => setCallState("answered")}>
                    <Box bg="green.500" borderRadius="full" p={4} shadow={3}>
                      <Icon
                        as={MaterialCommunityIcons}
                        name="phone"
                        size="xl"
